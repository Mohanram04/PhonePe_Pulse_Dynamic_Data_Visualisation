...
#sql connection
mydb = pymysql.connect(host="localhost", user="root", password="mohanram0305", database="phone_pe_1", port=3306, cursorclass=pymysql.cursors.DictCursor)
cursor = mydb.cursor()
...
if select == "Home":
    ...
    with col2:
        st.video("F:/phonepe_/Phone Pe Ad(720P_HD).mp4")

    col3,col4= st.columns(2)
    
    with col3:
        st.video("F:/phonepe_/PhonePe Motion Graphics(720P_HD).mp4")
    ...
    with col6:
        st.video("F:/phonepe_/PhonePe Motion Graphics(720P_HD)_2.mp4")
